...
// Fix sticky active states on mobile
function initMobileTouchFix() {
    ...
    if (isTouchDevice) {
        ...
        const interactiveElements = document.querySelectorAll(
            ...
            '.setting-item, .toggle-switch, .list-item, .website, ' +
            '.close-settings, .default-language-button, .language-button'
        );

        interactiveElements.forEach(element => {
            // Clear active state after touch
            element.addEventListener('touchend', function() {
                this.style.opacity = '';
                // Force reflow to ensure style is cleared
                void this.offsetHeight;
            }, { passive: true });

            // Also clear on touchcancel (when touch is interrupted)
