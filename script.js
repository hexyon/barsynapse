...
// Simple mobile touch fix - clear stuck states only
function initMobileTouchFix() {
    const isTouchDevice = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
    if (!isTouchDevice) return;
    
    // Simple cleanup for all interactive elements
    const allInteractive = document.querySelectorAll(
        'button, a, .toggle-switch, .setting-item, .list-item'
    );
...
// Set up proper click handlers for toggle switches
function setupToggleSwitches() {
    // Dark Mode Toggle - find it specifically
    ...
    settingItems.forEach((item, index) => {
        ...
        // Determine which toggle this is based on the label text
        const labelText = item.querySelector('.setting-label span');
        if (!labelText) return;
        
        const text = labelText.textContent.trim();
        
        // Add new click handler based on what toggle it is
        toggle.addEventListener('click', function(e) {
            ...
            if (text === 'Dark Mode') {
                toggleTheme(e);
            } else if (text === 'Change Icon') {
                toggleIcon(e);
            }
        }, { capture: true });
        
        // Block the setting-item from interfering
