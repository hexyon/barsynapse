...
function toggleTheme(event) {
    ...
    // Update ARIA attributes
    const toggleButton = document.querySelector('.toggle-switch:not(#icon-toggle):not(#design-toggle)');
    if (toggleButton) {
...
// Fix sticky active states on mobile - completely isolate each toggle
function initMobileTouchFix() {
    ...
    // Handle Dark Mode toggle completely separately
    const darkModeToggle = document.querySelector('.toggle-switch:not(#icon-toggle):not(#design-toggle)');
    if (darkModeToggle) {
        let darkModeTouch = null;
        
        darkModeToggle.addEventListener('touchstart', function(e) {
            e.stopPropagation();
            e.stopImmediatePropagation();
            darkModeTouch = e.touches[0].identifier;
        }, { passive: true });
        ...
        darkModeToggle.addEventListener('touchcancel', function(e) {
            e.stopPropagation();
            e.stopImmediatePropagation();
            darkModeTouch = null;
            this.style.opacity = '';
    ...
    // Handle Icon toggle completely separately
    const iconToggle = document.getElementById('icon-toggle');
    if (iconToggle) {
        let iconTouch = null;
        
        iconToggle.addEventListener('touchstart', function(e) {
            e.stopPropagation();
            e.stopImmediatePropagation();
            iconTouch = e.touches[0].identifier;
        }, { passive: true });
        
        iconToggle.addEventListener('touchend', function(e) {
            e.stopPropagation();
            e.stopImmediatePropagation();
            iconTouch = null;
            // Clear any stuck styles
        ...
        iconToggle.addEventListener('touchcancel', function(e) {
            e.stopPropagation();
            e.stopImmediatePropagation();
            iconTouch = null;
            this.style.opacity = '';
    ...
    // Handle Design toggle completely separately
    const designToggle = document.getElementById('design-toggle');
    if (designToggle) {
        let designTouch = null;
        
        designToggle.addEventListener('touchstart', function(e) {
            e.stopPropagation();
            e.stopImmediatePropagation();
            designTouch = e.touches[0].identifier;
        }, { passive: true });
        
        designToggle.addEventListener('touchend', function(e) {
            e.stopPropagation();
            e.stopImmediatePropagation();
            designTouch = null;
            // Clear any stuck styles
        ...
        designToggle.addEventListener('touchcancel', function(e) {
            e.stopPropagation();
            e.stopImmediatePropagation();
            designTouch = null;
            this.style.opacity = '';
    ...
    // Prevent setting items from capturing touches meant for toggles
    const settingItems = document.querySelectorAll('.setting-item');
    settingItems.forEach(item => {
        item.addEventListener('touchstart', function(e) {
            // Block all touch events on the container if not on a toggle
        ...
        item.addEventListener('click', function(e) {
            // Block all clicks on the container if not on a toggle
            if (!e.target.closest('.toggle-switch')) {
                e.preventDefault();
                e.stopPropagation();
            }
    ...
    const otherElements = document.querySelectorAll(
        '.header-button, .visit-button, .detail-button, ' +
        '.nav-button, .language-option, .settings-button, ' +
        '.close-settings, .default-language-button, .language-button'
    );
